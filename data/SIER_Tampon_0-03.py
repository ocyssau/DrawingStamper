...
class MonProgramme(Tk):
        ...
        def __init__(self):
                ...
                self.liste        = ('NOT for manufacturing \nOnly for quotation', 'OK for manufacturing')
        ...
        def _validate(self):
               # print self.RepIn_value.get(),self.RepOut_value.get(),self.Txt_value.get(),self.x_value.get(),self.y_value.get()
                ...
                if self.Txt_value.get() == 'OK for manufacturing':
                        col=(0,1,0)
                else:
                        col=(1,0,0)
                Wat=Wt.WaterRep(self.RepIn_value.get(),self.RepOut_value.get(),self.Txt_value.get(),float(self.x_value.get()),float(self.y_value.get()),self.StatusValue,RGB=col)
                if Wat.status==0:
                      self.destroy()  
        def _cancel(self):
                self.destroy()
